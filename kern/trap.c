...
trap_init_percpu(void)
{
	...
	//debug	cprintf("KSTACKTOP - i * (KSTKSIZE + KSTKGAP)- KSTKSIZE: %x\n", KSTACKTOP - i * (KSTKSIZE + KSTKGAP)- KSTKSIZE);
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if (curenv->env_pgfault_upcall!=NULL)
	{
		...
		if (tf->tf_esp >= UXSTACKTOP-PGSIZE && tf->tf_esp<UXSTACKTOP){
			user_mem_assert(curenv, (void *)tf->tf_esp - sizeof(struct UTrapframe) - 4, sizeof(struct UTrapframe) + 4,  PTE_U | PTE_W);
			tf->tf_eip = (int)curenv->env_pgfault_upcall;
			tf->tf_esp = tf->tf_esp - sizeof(struct UTrapframe) - 4;
			*(struct UTrapframe *)(tf->tf_esp) = utf;
			env_run(curenv);
		}
		else{
			user_mem_assert(curenv, (void *)UXSTACKTOP - sizeof(struct UTrapframe), sizeof(struct UTrapframe),  PTE_U | PTE_W);
			tf->tf_eip = (int)curenv->env_pgfault_upcall;
